...
public class JpaRecordRevisionDisplayReader implements RecordRevisionDisplayReader {
    ...
    @Override
    public Optional<DisplayedRevision> findCompleted(UUID finalRevisionId) {
        if (finalRevisionId == null) return Optional.empty();
        return revision(finalRevisionId);
    }

    @Override
    public Optional<DisplayedRevision> findInReview(UUID currentReviewId) {
        if (currentReviewId == null) return Optional.empty();
        List<?> ids =
                entityManager
                        .createNativeQuery("SELECT revision_id FROM reviews WHERE id=:id")
                        .setParameter("id", currentReviewId.toString())
                        .getResultList();
    ...
    private Optional<DisplayedRevision> revision(UUID id) {
        List<?> rows =
                entityManager
                        .createNativeQuery(
                                "SELECT id,revision_no,snapshot_json FROM record_revisions WHERE id=:id")
                        .setParameter("id", id.toString())
                        .getResultList();
        if (rows.isEmpty()) return Optional.empty();
        Object[] row = (Object[]) rows.get(0);
        return Optional.of(
                new DisplayedRevision(
