...
public class DemoAgentDataSeeder {
    ...
    private final JdbcTemplate jdbc;private final AgentRunService service;private final AgentRunWorker worker;private final AgentProperties properties;
    public DemoAgentDataSeeder(JdbcTemplate jdbc,AgentRunService service,AgentRunWorker worker,AgentProperties properties){this.jdbc=jdbc;this.service=service;this.worker=worker;this.properties=properties;}

    @EventListener(ApplicationReadyEvent.class)
    @Order(100)
    public void seed(){if(!"fake".equals(properties.getProvider()))return;List<String> projects=jdbc.queryForList("SELECT id FROM projects WHERE name=?",String.class,"HepG2 缺氧响应 qPCR");List<String> owners=jdbc.queryForList("SELECT id FROM users WHERE email_normalized=?",String.class,"member@example.com");if(projects.isEmpty()||owners.isEmpty())return;UUID project=UUID.fromString(projects.get(0)),owner=UUID.fromString(owners.get(0));ensureTerminal(owner,service.createProject(owner,project,new AgentDtos.CreateRunRequest("PROJECT_PROGRESS",Instant.parse("2026-07-01T00:00:00Z"),Instant.parse("2026-07-27T00:00:00Z"),"演示项目进展"),"demo:phase2:project-progress"));ensureTerminal(owner,service.createProject(owner,project,new AgentDtos.CreateRunRequest("PROJECT_PROGRESS",Instant.parse("2026-07-01T00:00:00Z"),Instant.parse("2026-07-27T00:00:00Z"),"__FAKE_SCENARIO__:evidence-invalid"),"demo:phase2:failed-run"));}
