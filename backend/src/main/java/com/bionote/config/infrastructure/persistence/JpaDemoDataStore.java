...
public class JpaDemoDataStore implements DemoDataStore {
    ...
    public Optional<UUID> findUserIdByEmail(String normalizedEmail) {
        return firstUuid("SELECT id FROM users WHERE email_normalized=:email", "email", normalizedEmail);
    }

    @Override
    public Optional<String> findEmailByUserId(UUID userId) {
        List<?> values = entityManager.createNativeQuery("SELECT email_normalized FROM users WHERE id=:id")
    ...
    public Optional<UUID> findRecordCreatorId(UUID recordId) {
        return firstUuid("SELECT creator_id FROM experiment_records WHERE id=:id", "id", recordId.toString());
    }
    ...
    @Override
    @Transactional
    public void alignRecordTimes(UUID recordId, Instant createdAt, Instant updatedAt, Instant attachmentCreatedAt) {
        entityManager.createNativeQuery("UPDATE experiment_records SET created_at=:createdAt,updated_at=:updatedAt WHERE id=:id")
                ...
                .executeUpdate();
        entityManager.createNativeQuery("UPDATE attachments SET created_at=:createdAt WHERE record_id=:recordId")
    ...
    public void spreadProjectTimeline(UUID projectId, Instant start, long stepHours) {
        ...
        for (int i = 0; i < eventIds.size(); i++) {
            ...
            entityManager.createNativeQuery("UPDATE audit_events SET created_at=:createdAt WHERE id=:id")
                    .setParameter("createdAt", Timestamp.from(last))
                    .setParameter("id", eventIds.get(i).toString())
                    .executeUpdate();
        }
        entityManager.createNativeQuery("UPDATE projects SET created_at=:createdAt,updated_at=:updatedAt WHERE id=:id")
