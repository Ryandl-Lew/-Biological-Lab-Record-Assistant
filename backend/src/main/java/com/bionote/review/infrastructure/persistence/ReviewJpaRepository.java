...
interface ReviewJpaRepository extends JpaRepository<ReviewEntity, UUID> {
    ...
    @Query(
            "update ReviewEntity r set r.status=:status,r.decisionComment=:comment,r.decidedAt=:decidedAt where r.id=:id and r.status='PENDING'")
    int decidePending(
            @Param("id") UUID id,
    ...
    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query(
            "update ReviewEntity r set r.reviewerId=:next where r.id=:id and r.reviewerId=:current and r.status='PENDING'")
    int reassignPending(
            @Param("id") UUID id, @Param("current") UUID current, @Param("next") UUID next);
    ...
    @Query(
            ...
            """,
            nativeQuery = true)
    List<PendingAssignmentProjection> pendingAssignments(
            @Param("projectId") String projectId, @Param("reviewerId") String reviewerId);
}
