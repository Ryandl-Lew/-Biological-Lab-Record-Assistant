...
public class UserController {
    ...
    public UserController(UserUseCase service){this.service=service;}
    ...
    @GetMapping("/{userId}/avatar") ResponseEntity<byte[]> avatar(@PathVariable UUID userId) throws Exception {
        ...
        return ResponseEntity.ok().cacheControl(CacheControl.noCache()).contentType(MediaType.parseMediaType(avatar.mediaType()))
                .body(avatar.bytes());
    }
