...
public class NotificationController {
    private final NotificationUseCase service; public NotificationController(NotificationUseCase service){this.service=service;}
    private UUID current(Authentication a){return UUID.fromString(a.getName());}
    @GetMapping PagedResponse<NotificationDtos.View> list(Authentication a,@RequestParam(defaultValue="false") boolean unreadOnly,@RequestParam(defaultValue="0") int page,@RequestParam(defaultValue="20") int size){return service.list(current(a),unreadOnly,page,size);}
