...
public class AttachmentController {
    private final AttachmentUseCase service;public AttachmentController(AttachmentUseCase service){this.service=service;}private UUID current(Authentication a){return UUID.fromString(a.getName());}
    ...
    private ResponseEntity<byte[]> payload(AttachmentUseCase.FilePayload p,String disposition){String encoded=UriUtils.encode(p.filename(),StandardCharsets.UTF_8).replace("+","%20");MediaType mediaType=MediaType.parseMediaType(p.mediaType());if("text".equals(mediaType.getType()))mediaType=new MediaType(mediaType,StandardCharsets.UTF_8);return ResponseEntity.ok().contentType(mediaType).header(HttpHeaders.CONTENT_DISPOSITION,disposition+"; filename*=UTF-8''"+encoded).header("X-Content-Type-Options","nosniff").body(p.bytes());}
}
