...
/**
 * Preset fit equations and natural-language aliases.
 * Custom univariate formulas are also supported via {@link #extractCustomEquation(String)}
 * when the user writes an explicit {@code y=...} expression.
 */
public final class FitMethodCatalog {
    public record Method(String id, String label, String equation) {}

    private static final Pattern CUSTOM_EQUATION = Pattern.compile(
            "(?i)y\\s*=\\s*([0-9a-zA-Z_+\\-*/^().,\\s]+)");
    ...
    public static String resolveEquation(String phrase) {
        ...
        for (Method method : METHODS) {
            ...
            if (text.equals(method.id()) || text.contains(method.label().toLowerCase(Locale.ROOT))) return method.equation();
        }
    ...
    /**
     * Extract and validate a user-written univariate formula such as {@code y=a*x/(b+x)}.
     * Returns normalized {@code y=...} or null.
     */
    public static String extractCustomEquation(String message) {
        ...
        while (matcher.find()) {
            ...
            try {
                ExpressionParser.CompiledEquation compiled = ExpressionParser.parseEquation(candidate);
    ...
    public static boolean wantsAutoCompare(String message) {
        ...
        return text.contains("不确定") || text.contains("不知道") || text.contains("随便")
                || text.contains("自动") || text.contains("试试看") || text.contains("试一下")
                || text.contains("比选") || text.contains("比较模型") || text.contains("选最优")
                || text.contains("auto") || text.contains("不确定方程") || text.contains("方程不确定");
    }

    public static String catalogDescription() {
        StringBuilder sb = new StringBuilder();
        sb.append("Preset univariate equations (also accept custom y=... with params a-z, x, exp/log/ln/sqrt/sin/cos, + - * / ^):\n");
        for (Method method : METHODS) {
            sb.append("- id=").append(method.id()).append(" label=").append(method.label())
                    .append(" equation=").append(method.equation()).append('\n');
        }
        return sb.toString();
    }
