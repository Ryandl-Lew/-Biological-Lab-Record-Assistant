...
public class CurveFitEngine {
    ...
    public FitModels.FitResult fitBest(
        ...
        if (points == null || points.size() < 3) {
            throw new ApiException(
                    HttpStatus.BAD_REQUEST, "FIT_INSUFFICIENT_POINTS", "至少需要 3 个有效数据点才能拟合");
        }
        ...
        for (ExpressionParser.CompiledEquation equation : equations) {
            try {
                ...
                comparisons.add(
                        new FitModels.ModelComparison(
                                equation.equation(), Double.NaN, Double.NaN, 0, false));
            }
        }
        if (successes.isEmpty()) {
            throw new ApiException(
                    HttpStatus.UNPROCESSABLE_ENTITY, "FIT_FAILED", "所有候选方程均未能完成拟合，请检查数据或缩小候选范围");
        }
        successes.sort(
                Comparator.comparingDouble(FitModels.FitResult::rSquared)
                        .reversed()
            ...
            for (int i = 0; i < marked.size(); i++) {
                ...
                if (row.equation().equals(best.equation()) && Double.isFinite(row.rSquared())) {
                    marked.set(
                            i,
                            new FitModels.ModelComparison(
                                    row.equation(), row.rSquared(), row.rmse(), row.n(), true));
                    break;
                }
            }
        }
    ...
    private FitModels.FitResult fitInternal(
            ExpressionParser.CompiledEquation equation,
            List<FitModels.DataPoint> points,
            List<FitModels.SkipInfo> skipped,
        ...
        if (observations.toList().size() < 3) {
            throw new ApiException(
                    HttpStatus.BAD_REQUEST, "FIT_INSUFFICIENT_POINTS", "至少需要 3 个有限数据点");
        }
        ...
        try {
            fitted =
                    SimpleCurveFitter.create(function, guess)
                            .withMaxIterations(200)
                            .fit(observations.toList());
        } catch (Exception e) {
            throw new ApiException(
                    HttpStatus.UNPROCESSABLE_ENTITY, "FIT_FAILED", "曲线拟合未收敛，请检查方程或数据");
        }
        ...
        for (FitModels.DataPoint point : points) {
            if (point.recordCode() != null && !used.contains(point.recordCode()))
                used.add(point.recordCode());
    ...
    private List<FitModels.DataPoint> filterForEquation(
            ExpressionParser.CompiledEquation equation, List<FitModels.DataPoint> points) {
        String rhs = equation.rhs() == null ? "" : equation.rhs().toLowerCase();
        boolean needsPositiveX =
                rhs.contains("log(") || rhs.contains("ln(") || rhs.contains("sqrt(");
