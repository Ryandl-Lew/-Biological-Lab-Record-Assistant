...
public class FitIntentParser {
    private static final Pattern EQUATION =
            Pattern.compile("(?i)(?:方程\\s*[:：]\\s*)?(y\\s*=\\s*[^;；\\n]+)");
    private static final Pattern X_SPEC =
            Pattern.compile(
    ...
    public FitModels.FitIntent parse(String message) {
        ...
        if (statusScoped) {
            for (String status :
                    List.of("COMPLETED", "IN_PROGRESS", "IN_REVIEW", "CHANGES_REQUESTED")) {
        ...
        Matcher typeMatcher =
                Pattern.compile("(?i)(?:实验类型|类型)\\s*[=:：]?\\s*([\\w\\u4e00-\\u9fff]+)")
                        .matcher(message);
        ...
        String csvHint = null;
        Matcher csvMatcher =
                Pattern.compile("(?i)(?:csv|xlsx|excel)\\s*[=:：]?\\s*([\\w.\\-]+)")
                        .matcher(message);
        if (csvMatcher.find()) csvHint = csvMatcher.group(1);

        String missing = null;
        if (equation == null && (xSpec == null || ySpec == null))
            missing = "请指定拟合方程（如 y = a + b*x）或自变量/因变量列名（如 xField=concentration yField=ct）";
        else if (equation != null && (xSpec == null || ySpec == null))
            missing = "请指定自变量与因变量，例如：xField=concentration，yField=ct；或 CSV 列名/列号";
    ...
    private FitModels.FitIntent tryJson(String message) {
        ...
        try {
            ...
            List<String> codes = new ArrayList<>();
            if (node.path("recordCodes").isArray())
                node.path("recordCodes").forEach(v -> codes.add(v.asText()));
            List<String> statuses = new ArrayList<>();
            if (node.path("statuses").isArray())
                node.path("statuses").forEach(v -> statuses.add(v.asText()));
