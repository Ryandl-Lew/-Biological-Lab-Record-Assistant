...
public class AgentToolExecutor {
    private final AgentToolRegistry registry; private final ObjectMapper json; private final Validator validator;private final AgentRunStore runs;private final PromptVersionService prompts;
    public AgentToolExecutor(AgentToolRegistry registry,ObjectMapper json,Validator validator,AgentRunStore runs,PromptVersionService prompts){this.registry=registry;this.json=json;this.validator=validator;this.runs=runs;this.prompts=prompts;}
    ...
    public ExecutedToolResult execute(AgentToolContext context,String name,JsonNode arguments,Set<String> allowedTools,AgentRunMemory memory){
        ...
        int perToolLimit=perToolLimit(context.runId(),name);if(memory.toolCalls(name)>=perToolLimit)throw new ApiException(HttpStatus.TOO_MANY_REQUESTS,"AGENT_LIMIT_EXCEEDED","Per-tool call limit exceeded for "+name);
    ...
    private int perToolLimit(java.util.UUID runId,String name){try{JsonNode value=prompts.get(runs.load(runId).promptVersionId()).toolPolicy().path("maxCallsByTool").path(name);return value.isInt()&&value.asInt()>0?value.asInt():Integer.MAX_VALUE;}catch(Exception e){return Integer.MAX_VALUE;}}
