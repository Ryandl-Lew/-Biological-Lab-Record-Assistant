...
public interface BioNoteAgentQueryStore {
    PageSlice listRecords(UUID projectId, Instant start, Instant end, List<String> statuses, int page, int size);
    ...
    List<Map<String,Object>> listRevisions(UUID recordId, int maxRevisionNo, Instant start, Instant end);
    List<Map<String,Object>> listReviews(UUID recordId, int maxRevisionNo);
    PageSlice activity(UUID projectId, Instant start, Instant end, List<String> eventTypes, int page, int size);
    ...
    Optional<UUID> findRevisionRecord(UUID revisionId, UUID projectId, int maxRevisionNo, Instant through);
