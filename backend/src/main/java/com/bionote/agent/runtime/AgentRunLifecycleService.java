...
public class AgentRunLifecycleService {
    ...
    public AgentRunLifecycleService(
            AgentRunStore runs,
            AgentArtifactAppender artifacts,
            AgentRunStateMachine states,
    ...
    public void finish(UUID runId, String status, String code, String message) {
        ...
        if (changed != 1)
            throw new IllegalStateException("Agent run is no longer RUNNING: " + runId);
    }
    ...
    public UUID completeWithArtifact(UUID runId, JsonNode content, JsonNode evidence) {
        ...
        Instant now = Instant.now();
        artifacts.append(
                new AgentArtifactAppender.ArtifactRecord(
                        artifactId,
                        runId,
                        run.artifactKind(),
                        run.projectId(),
    ...
    public int failStaleRunning(Instant cutoff) {
        return runs.failStaleRunning(cutoff, Instant.now());
    }
    ...
    private ApiException cancelled() {
        return new ApiException(
                HttpStatus.CONFLICT,
                "AGENT_RUN_CANCELLED",
                "Agent run was cancelled before artifact persistence");
    }
