...
public class JdbcBioNoteAgentQueryStore implements BioNoteAgentQueryStore {
    ...
    @Override public List<Map<String,Object>> listAttachments(UUID recordId) {return jdbc.queryForList("SELECT id,record_id,original_filename,media_type,size_bytes,previewable,created_at FROM attachments WHERE record_id=? AND deleted_at IS NULL ORDER BY created_at DESC LIMIT 50",recordId.toString());}
    @Override public List<Map<String,Object>> listProjectAttachments(UUID projectId) {return jdbc.queryForList("SELECT a.id,a.record_id,a.original_filename,a.media_type,a.size_bytes,a.previewable,a.created_at,r.code record_code,r.title record_title FROM attachments a JOIN experiment_records r ON r.id=a.record_id WHERE r.project_id=? AND a.deleted_at IS NULL AND r.deleted_at IS NULL ORDER BY a.created_at DESC LIMIT 100",projectId.toString());}
    @Override public Optional<Map<String,Object>> findAttachment(UUID attachmentId,UUID recordId) {return jdbc.queryForList("SELECT id,record_id,original_filename,media_type,size_bytes,storage_key,previewable FROM attachments WHERE id=? AND record_id=? AND deleted_at IS NULL",attachmentId.toString(),recordId.toString()).stream().findFirst();}
    @Override public Optional<Map<String,Object>> findAnyAttachment(UUID attachmentId) {return jdbc.queryForList("SELECT id,record_id,original_filename,media_type,size_bytes,storage_key,previewable FROM attachments WHERE id=? AND deleted_at IS NULL",attachmentId.toString()).stream().findFirst();}
    ...
    @Override public Optional<Map<String,Object>> findRecord(UUID recordId,UUID projectId) {return jdbc.queryForList("SELECT r.*,u.display_name creator_name FROM experiment_records r JOIN users u ON u.id=r.creator_id WHERE r.id=? AND r.project_id=? AND r.deleted_at IS NULL",recordId.toString(),projectId.toString()).stream().findFirst();}
