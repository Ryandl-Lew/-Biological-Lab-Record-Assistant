...
public class JdbcAgentChatContextStore implements AgentChatContextStore {
    ...
    @Override public List<Map<String,Object>> activeAttachments(UUID recordId) {return jdbc.queryForList("SELECT original_filename, content_type, size_bytes FROM attachments WHERE record_id=? AND deleted_at IS NULL ORDER BY created_at DESC LIMIT 10",recordId.toString());}
    ...
    @Override public List<Map<String,Object>> recentRecords(UUID projectId) {return jdbc.queryForList("SELECT code, title, status, current_revision_no, updated_at FROM experiment_records WHERE project_id=? AND deleted_at IS NULL AND provisional=FALSE ORDER BY updated_at DESC LIMIT 30",projectId.toString());}
