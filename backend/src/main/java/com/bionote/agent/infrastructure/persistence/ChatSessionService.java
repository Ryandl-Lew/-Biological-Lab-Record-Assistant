...
public class ChatSessionService implements ChatSessionStore {
    ...
    public ChatSessionService(JdbcTemplate jdbc) { this.jdbc = jdbc; }
    ...
    public List<AgentDtos.ChatSessionSummary> listByProject(UUID userId, UUID projectId) {
        ...
        ).stream().map(row -> new AgentDtos.ChatSessionSummary(
    ...
    public List<AgentDtos.ChatSessionSummary> listByRecord(UUID userId, UUID recordId) {
        return jdbc.queryForList(
            ...
            userId.toString(), recordId.toString()
        ).stream().map(row -> new AgentDtos.ChatSessionSummary(
            UUID.fromString(row.get("id").toString()),
            String.valueOf(row.get("title")),
            ((java.sql.Timestamp) row.get("created_at")).toInstant(),
    ...
    public AgentDtos.ChatSessionDetail get(UUID userId, UUID sessionId) {
        ...
        return new AgentDtos.ChatSessionDetail(
            ...
            session.get("project_id") != null ? UUID.fromString(session.get("project_id").toString()) : null,
            session.get("record_id") != null ? UUID.fromString(session.get("record_id").toString()) : null,
    ...
    @Transactional
    public AgentDtos.ChatSessionDetail save(UUID userId, UUID projectId, UUID recordId, AgentDtos.SaveSessionRequest request) {
        UUID sessionId = UUID.randomUUID();
        Instant now = Instant.now();
        String title = request.title() != null && !request.title().isBlank() ? request.title() : "新对话";
        jdbc.update(
            "INSERT INTO agent_chat_sessions (id, project_id, record_id, user_id, title, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
            sessionId.toString(), projectId != null ? projectId.toString() : null,
            recordId != null ? recordId.toString() : null, userId.toString(), title, now, now
        );
    ...
    public AgentDtos.ChatSessionDetail appendMessages(UUID userId, UUID sessionId, List<AgentDtos.ChatMessage> messages) {
        Map<String, Object> existing = jdbc.queryForMap(
            "SELECT id FROM agent_chat_sessions WHERE id = ? AND user_id = ?", sessionId.toString(), userId.toString()
        );
