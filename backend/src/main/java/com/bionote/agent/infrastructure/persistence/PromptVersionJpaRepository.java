...
interface PromptVersionJpaRepository extends JpaRepository<PromptVersionEntity, UUID> {
    ...
    @Query("update PromptVersionEntity p set p.active=false,p.activeNameKey=null where p.promptName=:name and p.active=true")
    int deactivateAll(@Param("name") String name);

    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("update PromptVersionEntity p set p.active=false,p.activeNameKey=null where p.promptName=:name and p.id<>:id and p.active=true")
