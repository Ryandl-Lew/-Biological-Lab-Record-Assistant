...
public class JsonSchemaArtifactValidator implements AgentResultValidator {
    ...
    public JsonSchemaArtifactValidator(EvidenceValidator evidence) {
        this.evidence = evidence;
    }

    @Override
    public ValidationResult validate(AgentRunContext context, JsonNode candidate, JsonNode schema) {
        List<String> errors = new ArrayList<>();
        check("$", candidate, schema, errors);
        if (errors.isEmpty()
                && Set.of("record-summary", "project-progress").contains(context.prompt().name()))
            errors.addAll(evidence.validate(context, candidate));
        return new ValidationResult(errors.isEmpty(), errors);
    }

    private void check(String path, JsonNode value, JsonNode schema, List<String> errors) {
        ...
        if ("object".equals(type)) {
                        ...
                        .forEachRemaining(
                                name -> {
                                    if (!allowed.contains(name))
                                        errors.add(path + " contains unknown property " + name);
                                });
            }
            ...
            while (names.hasNext()) {
                ...
                if (value.has(name))
                    check(path + "." + name, value.get(name), properties.get(name), errors);
            }
            ...
            for (JsonNode item : value)
                check(path + "[" + (i++) + "]", item, schema.path("items"), errors);
        } else if ("string".equals(type)) {
            if (value.asText().length() < schema.path("minLength").asInt(0))
                errors.add(path + " is too short");
            if (schema.has("maxLength")
                    && value.asText().length() > schema.path("maxLength").asInt())
                errors.add(path + " is too long");
            if (schema.has("enum")) {
                boolean found = false;
                for (JsonNode option : schema.path("enum"))
                    if (option.asText().equals(value.asText())) found = true;
