...
public class EvidenceValidator {
    ...
    private final ProjectMemberStore members;private final RecordStore records;private final RevisionLookup revisions;private final ReviewStore reviews;private final AuditQueryStore audit;private final ObjectMapper json;
    public EvidenceValidator(ProjectMemberStore members,RecordStore records,RevisionLookup revisions,ReviewStore reviews,AuditQueryStore audit,ObjectMapper json){this.members=members;this.records=records;this.revisions=revisions;this.reviews=reviews;this.audit=audit;this.json=json;}
    ...
    private void validateObject(AgentRunContext c,EvidenceCandidate e,List<String> errors){try{switch(e.type()){case "PROJECT"->{if(!e.id().equals(c.run().projectId().toString()))errors.add("EVIDENCE: project mismatch");}case "RECORD"->{if(!activeRecordInProject(UUID.fromString(e.id()),c.run().projectId()))errors.add("EVIDENCE: record is inaccessible");}case "REVISION"->{var revision=revisions.findById(UUID.fromString(e.id()));if(revision.isEmpty()||!activeRecordInProject(revision.get().recordId(),c.run().projectId()))errors.add("EVIDENCE: revision is inaccessible");}case "REVIEW"->{var review=reviews.findById(UUID.fromString(e.id()));if(review.isEmpty()||!activeRecordInProject(review.get().recordId(),c.run().projectId()))errors.add("EVIDENCE: review is inaccessible");}case "AUDIT_EVENT"->{if(!audit.exists(UUID.fromString(e.id()),c.run().projectId()))errors.add("EVIDENCE: audit event is inaccessible");}case "REVISION_DIFF"->{if(e.fromRevisionId()==null||e.toRevisionId()==null||e.recordId()==null){errors.add("EVIDENCE: invalid revision diff");break;}var from=revisions.findById(e.fromRevisionId());var to=revisions.findById(e.toRevisionId());if(from.isEmpty()||to.isEmpty()||!from.get().recordId().equals(e.recordId())||!to.get().recordId().equals(e.recordId())||!activeRecordInProject(e.recordId(),c.run().projectId())){errors.add("EVIDENCE: revision diff sources are inaccessible");break;}String actual=sha(from.get().contentHash()+":"+to.get().contentHash());if(!actual.equals(e.sourceHash()))errors.add("EVIDENCE: revision diff source hash changed");}default->errors.add("EVIDENCE: unsupported type "+e.type());}}catch(Exception ex){errors.add("EVIDENCE: object validation failed");}}
    private boolean activeRecordInProject(UUID recordId,UUID projectId){return records.findActive(recordId).map(value->value.projectId().equals(projectId)).orElse(false);}
    private boolean member(UUID project,UUID actor){return members.findRole(project,actor).isPresent();}
