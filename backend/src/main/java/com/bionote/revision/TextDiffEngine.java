...
public class TextDiffEngine {
    public Result diff(String before, String after, int maxOperations) {
        ...
        if (left.equals(right))
            return new Result(List.of(new RevisionDtos.TextOperation("EQUAL", left)), false);
        ...
        while (i < a.length || j < b.length) {
            if (i < a.length && j < b.length && a[i].equals(b[j])) {
                append(operations, "EQUAL", a[i]);
                i++;
                j++;
            } else if (j < b.length && (i == a.length || lcs[i][j + 1] >= lcs[i + 1][j])) {
                append(operations, "INSERT", b[j++]);
            } else {
                append(operations, "DELETE", a[i++]);
            }
            if (operations.size() > maxOperations)
                return new Result(List.copyOf(operations.subList(0, maxOperations)), true);
        }
    ...
    private Result coarse(String before, String after, int maxOperations, boolean truncated) {
        ...
        if (!after.isEmpty() && operations.size() < maxOperations)
            operations.add(new RevisionDtos.TextOperation("INSERT", after));
    ...
    private void append(List<RevisionDtos.TextOperation> operations, String type, String value) {
        if (!operations.isEmpty()
                && operations.get(operations.size() - 1).operation().equals(type)) {
