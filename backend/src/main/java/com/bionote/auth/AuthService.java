...
public class AuthService implements AuthUseCase {
    ...
    public AuthDtos.AuthResult register(AuthDtos.RegisterRequest request) {
        ...
        if (users.existsByEmailNormalized(email))
            throw new ApiException(
                    HttpStatus.CONFLICT, "DUPLICATE_RESOURCE", "该邮箱已注册", Map.of("email", "该邮箱已注册"));
        User user =
                new User(
                        UUID.randomUUID(),
    ...
    @Override
    public com.bionote.user.UserDtos.UserView currentUser(UUID userId) {
        return view(
    ...
    private com.bionote.user.UserDtos.UserView view(User user) {
        return new com.bionote.user.UserDtos.UserView(
                user.getId(),
                user.getDisplayName(),
                user.getEmailNormalized(),
                user.getAvatarStorageKey() == null
                        ? null
