...
interface RestoreOperationJpaRepository extends JpaRepository<RestoreOperationEntity,UUID> {
    interface StoredProjection {String getId();String getRecordId();String getSourceRevisionId();int getRevisionNo();
        String getActorId();String getActorName();long getBeforeVersion();long getAfterVersion();String getBeforeHash();
        String getAfterHash();String getDiffSummaryJson();String getPayloadHash();Instant getRestoredAt();}
    @Query(value="""
            ...
            """,nativeQuery=true)
    Optional<StoredProjection> findStored(@Param("recordId") String recordId,@Param("key") String key);
    @Query(value="""
            SELECT o.id,o.record_id AS recordId,o.source_revision_id AS sourceRevisionId,
                   rv.revision_no AS revisionNo,o.actor_id AS actorId,u.display_name AS actorName,
                   o.before_record_version AS beforeVersion,o.after_record_version AS afterVersion,
            ...
            """,countQuery="SELECT COUNT(*) FROM record_restore_operations WHERE record_id=:recordId",nativeQuery=true)
