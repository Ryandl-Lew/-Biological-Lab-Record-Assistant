...
class ProjectInvitationEntity {
    ...
    @Column(name = "inviter_id", nullable = false) @JdbcTypeCode(SqlTypes.CHAR) UUID inviterId;
    @Column(name = "invitee_user_id", nullable = false) @JdbcTypeCode(SqlTypes.CHAR) UUID inviteeUserId;
    @Column(name = "invitee_email_snapshot", nullable = false, length = 255) String inviteeEmailSnapshot;
    @Column(nullable = false, length = 20) String status;
    @Column(name = "expires_at", nullable = false) Instant expiresAt;
    @Column(name = "created_at", nullable = false) Instant createdAt;
    @Column(name = "responded_at") Instant respondedAt;
    @Column(name = "pending_key", length = 100) String pendingKey;

    protected ProjectInvitationEntity() {}

    ProjectInvitationEntity(UUID id, UUID projectId, UUID inviterId, UUID inviteeUserId,
