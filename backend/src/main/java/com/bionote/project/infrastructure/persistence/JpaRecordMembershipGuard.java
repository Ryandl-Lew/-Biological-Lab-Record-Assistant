...
public class JpaRecordMembershipGuard implements RecordMembershipGuard {
    ...
    public JpaRecordMembershipGuard(JpaProjectPersistenceAdapter persistence) { this.persistence = persistence; }

    @Override public List<BlockingItem> recordsBlockingArchive(UUID projectId) {
        return persistence.blockingArchive(projectId).stream()
                .map(record -> new BlockingItem(record.id, record.title, record.status)).toList();
    }

    @Override public List<BlockingItem> recordsBlockingMemberChange(UUID projectId, UUID userId) {
