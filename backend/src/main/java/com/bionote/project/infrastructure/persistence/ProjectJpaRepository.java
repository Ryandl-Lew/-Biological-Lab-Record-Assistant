...
interface ProjectJpaRepository extends JpaRepository<ProjectEntity, UUID> {
    interface AccessProjection {
        String getId(); String getName(); String getDescription(); String getDetailedDescription(); String getStatus();
        String getOwnerId(); Instant getCreatedAt(); Instant getUpdatedAt(); Instant getArchivedAt(); long getVersion(); String getRole();
    }
            ...
            SELECT COUNT(*) FROM projects p JOIN project_members pm ON pm.project_id=p.id
             WHERE pm.user_id=:userId
               ...
               AND (LOWER(p.name) LIKE CONCAT('%',:keyword,'%') ESCAPE '!'
                    ...
                    OR LOWER(COALESCE(p.detailed_description,'')) LIKE CONCAT('%',:keyword,'%') ESCAPE '!')
            """, nativeQuery = true)
    Page<AccessProjection> searchForMember(@Param("userId") String userId, @Param("status") String status,
                                           @Param("keyword") String escapedKeyword, Pageable pageable);

    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("update ProjectEntity p set p.status='ARCHIVED',p.archivedAt=:now,p.updatedAt=:now,p.version=p.version+1 where p.id=:id and p.status='ACTIVE'")
