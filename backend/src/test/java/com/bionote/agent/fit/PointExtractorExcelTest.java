...
class PointExtractorExcelTest {
    private final PointExtractor extractor = new PointExtractor(mock(AgentFitStore.class), new ObjectMapper(), mock(AttachmentStorage.class));

    @Test
    void peeksHeadersFromFirstSheet() throws Exception {
        byte[] bytes = workbookBytes(true);
        assertThat(extractor.peekHeaders(bytes, "标曲.xlsx"))
                .containsExactly("log10_copies", "ct");
    }

    @Test
    void parsesFirstSheetByHeaderNames() throws Exception {
        byte[] bytes = workbookBytes(true);
        List<FitModels.DataPoint> points = extractor.parseXlsx(bytes, UUID.randomUUID(), "EXP-X", "log10_copies", "ct");
    ...
    void parsesFirstSheetByColumnIndexWithoutHeader() throws Exception {
        ...
        List<FitModels.DataPoint> points = extractor.parseXlsx(bytes, UUID.randomUUID(), "EXP-Y", "0", "1");
        assertThat(points).hasSize(3);
