...
class DomainEventIntegrationTest {
    ...
    void restoredEventWritesIdempotentSanitizedAuditRow() throws Exception {
        ...
        RecordRevisionRestoredEvent event =
        ...
        Map<String, Object> row =
                jdbc.queryForMap("SELECT * FROM audit_events WHERE id=?", eventId.toString());
        ...
        Map<String, Object> metadata =
                json.readValue(row.get("metadata_json").toString(), new TypeReference<>() {});
        assertThat(metadata)
                .containsEntry("revisionNo", 2)
                .containsEntry("sourceRevisionId", revisionId.toString())
                .containsEntry("fromVersion", 4)
                .containsEntry("toVersion", 5)
                .containsEntry("attachmentAdded", 1)
                .containsEntry("attachmentRemoved", 2);
        assertThat(metadata)
                .doesNotContainKeys("content", "prompt", "token", "storageKey", "previewToken");
    }
}
