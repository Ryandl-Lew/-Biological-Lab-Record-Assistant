...
test('phase2 revision, restore, artifact and trace authorization remains object scoped', async ({
  request,
}) => {
  ...
  record = await apiCall(request, 'PUT', `/records/${record.id}`, member.token, {
  ...
  const revision = await apiCall(
    ...
    member.token,
    { reviewerId: owner.user.id, expectedRecordVersion: record.version },
    { 'Idempotency-Key': unique('phase2-auth-submit') },
  )
  await apiCall(
    request,
    'POST',
    `/records/${record.id}/reviews/${revision.review.id}/request-changes`,
    owner.token,
    { comment: '退回以验证恢复权限' },
  )
  record = await apiCall(request, 'GET', `/records/${record.id}`, member.token)

  const outsiderRevision = await request.get(
    `${API_BASE}/records/${record.id}/revisions/${revision.id}`,
    { headers: { Authorization: `Bearer ${outsider.token}` } },
  )
  expect([403, 404]).toContain(outsiderRevision.status())
  const ownerRestore = await request.post(`${API_BASE}/records/${record.id}/restore-preview`, {
  ...
  const memberProjectRun = await request.post(`${API_BASE}/projects/${project.id}/agent-runs`, {
    headers: { Authorization: `Bearer ${member.token}`, 'Idempotency-Key': unique('member-agent') },
    data: { artifactKind: 'PROJECT_PROGRESS' },
  })
  ...
  const run = await apiCall(
    ...
    { 'Idempotency-Key': unique('owner-agent') },
  )
  let current = run
  ...
  const outsiderRun = await request.get(`${API_BASE}/agent-runs/${run.id}/steps`, {
    headers: { Authorization: `Bearer ${outsider.token}` },
  })
  expect([403, 404]).toContain(outsiderRun.status())
  const outsiderArtifact = await request.get(`${API_BASE}/agent-artifacts/${current.artifactId}`, {
    headers: { Authorization: `Bearer ${outsider.token}` },
  })
