...
test('optimistic lock rejects stale saves and submission key is idempotent', async ({
  request,
}) => {
  ...
  const first = await apiCall(request, 'PUT', `/records/${record.id}`, creator.token, {
  ...
  const firstSubmit = await request.post(`${API_BASE}/records/${record.id}/submissions`, {
    headers: { Authorization: `Bearer ${creator.token}`, 'Idempotency-Key': idempotencyKey },
    data: payload,
  })
  const secondSubmit = await request.post(`${API_BASE}/records/${record.id}/submissions`, {
    headers: { Authorization: `Bearer ${creator.token}`, 'Idempotency-Key': idempotencyKey },
    data: payload,
  })
