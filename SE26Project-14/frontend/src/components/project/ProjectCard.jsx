...
export default function ProjectCard({ project }) {
  ...
  return (
    <article className="flex flex-col rounded-xl border border-slate-200/60 bg-white/40 p-5 shadow-card backdrop-blur-sm hover:border-brand-200">
      ...
      <dl className="mt-4 grid grid-cols-3 divide-x divide-slate-200 rounded-xl border border-slate-100 bg-slate-50/80 px-2 py-3 text-center text-xs">
        <div>
          <dt className="text-slate-400">成员</dt>
          <dd className="mt-1 font-medium">{project.memberCount} 人</dd>
        </div>
        <div>
          <dt className="text-slate-400">记录</dt>
          <dd className="mt-1 font-medium">{project.recordCount} 条</dd>
        </div>
        <div>
          <dt className="text-slate-400">更新</dt>
          <dd className="mt-1 font-medium">{new Date(project.updatedAt).toLocaleDateString()}</dd>
        </div>
      </dl>
