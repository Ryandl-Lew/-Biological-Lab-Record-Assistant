...
describe('Agent experience', () => {
    ...
    render(
      <AgentArtifactView
        artifact={{
          ...
          content: {
            headline: '记录总结',
            executiveSummary: '已验证摘要',
            period: { start: 'a', end: 'b' },
            progress: [{ id: 'p1', statement: '版本已变化', evidenceRefs: ['e1'] }],
            risks: [{ id: 'risk', statement: '仍需复核', severity: 'HIGH', evidenceRefs: ['e1'] }],
            nextActions: [
              { id: 'a1', statement: '补充对照', basis: 'REVIEW_FEEDBACK', evidenceRefs: ['e1'] },
            ],
            evidence: [evidence],
            limitations: ['未读取附件正文'],
          },
  ...
  it('polls active runs, stops at success and reports the artifact', async () => {
    vi.useFakeTimers()
    fetchAgentRun
      .mockResolvedValueOnce({ id: 'run', status: 'QUEUED' })
      ...
      items: [
        {
          ...
          stepNo: 1,
          stepType: 'TOOL_CALL',
          toolName: 'get_record_overview',
          latencyMs: 4,
          request: { recordId: 'r1', apiKey: 'secret' },
          response: { count: 1, reasoning: 'hidden' },
        },
      ],
      meta: { page: 0 },
    })
  ...
  it('cancels active runs, reruns terminal failures and clears polling on unmount', async () => {
    ...
    fetchAgentRun.mockResolvedValue({
      id: 'failed',
      status: 'FAILED',
