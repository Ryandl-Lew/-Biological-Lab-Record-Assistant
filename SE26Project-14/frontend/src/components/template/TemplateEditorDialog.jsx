...
export default function TemplateEditorDialog({ template, onClose, onSave }) {
  ...
  const submit = async (event) => {
    ...
    if (
      form.fields.some(
        (field) => field.fieldType === 'SELECT' && !field.options.filter(Boolean).length,
      )
    ) {
      setError('下拉字段至少需要一个选项')
      return
    }
    setSaving(true)
    try {
      await onSave({
        ...
        fields: form.fields.map((field, index) => ({
          ...
          options: field.fieldType === 'SELECT' ? field.options.filter(Boolean) : [],
          defaultValue: field.fieldType === 'FILE' ? null : field.defaultValue,
        })),
      })
    } catch (requestError) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
      <form
        onSubmit={submit}
        className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white/95 p-6 shadow-pop backdrop-blur-sm"
      >
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <label htmlFor="template-experiment-type" className="field-label">
              实验类型
            </label>
            <input
              id="template-experiment-type"
        ...
        <div className="mt-6 flex items-center justify-between">
          <h3 className="font-semibold">动态字段</h3>
          <Button
            size="sm"
            variant="secondary"
            icon={Plus}
        ...
        <div className="mt-3 space-y-3">
          {form.fields.map((field, index) => (
            <div key={field.fieldKey} className="rounded-xl border border-slate-200 p-4">
              <div className="grid gap-3 md:grid-cols-[1.2fr,1fr,1.2fr,auto]">
                <input
                ...
                <select
                  ...
                  onChange={(e) => update(index, 'fieldType', e.target.value)}
                  className="input"
                >
                  {Object.entries(TEMPLATE_FIELD_TYPE_LABELS).map(([value, label]) => (
