...
const gutterClass = (side) =>
  `select-none border-r border-slate-200 px-2 text-right font-mono text-xs text-slate-500 ${side && TONE[side.tone].gutter ? TONE[side.tone].gutter : 'bg-slate-50'}`
...
function Value({ value }) {
  ...
  return (
    <pre className="whitespace-pre-wrap break-words text-xs">{JSON.stringify(value, null, 2)}</pre>
  )
}
...
const hunkLines = (hunks, side) =>
  toLines(
    (hunks || [])
      .filter(
        (hunk) =>
          hunk.operation === 'EQUAL' ||
          (side === 'before' ? hunk.operation === 'DELETE' : hunk.operation === 'INSERT'),
      )
...
function buildBodyRows(sections) {
  ...
  sections.forEach((section) => {
    if (section.status === 'ADDED') {
      plainLines(section.after, true).forEach((line) =>
        rows.push({ left: null, right: { line, tone: 'add' } }),
      )
    } else if (section.status === 'REMOVED') {
      plainLines(section.before, true).forEach((line) =>
        rows.push({ left: { line, tone: 'del' }, right: null }),
      )
    } else if (section.status === 'MODIFIED') {
      const leftLines = section.textHunks?.length
        ? hunkLines(section.textHunks, 'before')
        : plainLines(section.before, true)
      const rightLines = section.textHunks?.length
        ? hunkLines(section.textHunks, 'after')
        : plainLines(section.after, true)
      ...
      plainLines(section.before ?? section.after, false).forEach((line) =>
        rows.push({ left: { line, tone: 'context' }, right: { line, tone: 'context' } }),
      )
    }
  })
  return rows
...
function DiffLine({ line, tone }) {
  ...
  return line.map((segment, index) =>
    segment.changed ? (
      <mark key={index} className={`rounded-sm px-0.5 ${TONE[tone].mark}`}>
        {segment.text}
      </mark>
    ) : (
...
function SideBySideTable({ sections, from, to, ariaLabel }) {
  ...
  let rightNo = 0
  return (
    <table aria-label={ariaLabel} className="w-full table-fixed border-collapse text-sm leading-6">
      <colgroup>
        <col className="w-11" />
        <col />
        <col className="w-11" />
      ...
      <thead>
        <tr className="border-b border-slate-200 bg-slate-50 text-left text-xs text-slate-500">
          <th colSpan={2} className="border-r border-slate-200 px-3 py-2 font-medium">
            基准：{sourceLabel(from)}
          </th>
        ...
        {rows.map((row, rowIndex) => {
          ...
          return (
            <tr key={rowIndex} className="align-top">
              <td aria-hidden="true" className={gutterClass(row.left)}>
                {leftNumber ?? ''}
              </td>
              <td
                className={`whitespace-pre-wrap break-words border-r border-slate-200 px-3 ${row.left ? TONE[row.left.tone].cell : 'bg-slate-50'}`}
              >
                {row.left ? <DiffLine line={row.left.line} tone={row.left.tone} /> : ''}
              </td>
              <td aria-hidden="true" className={gutterClass(row.right)}>
                {rightNumber ?? ''}
              </td>
              <td
                className={`whitespace-pre-wrap break-words px-3 ${row.right ? TONE[row.right.tone].cell : 'bg-slate-50'}`}
              >
                {row.right ? <DiffLine line={row.right.line} tone={row.right.tone} /> : ''}
              </td>
            </tr>
          )
        })}
    ...
    <article className="overflow-hidden rounded-xl border border-slate-200">
      <header className="border-b border-slate-200 px-4 py-3">
        ...
        <p className="text-xs text-slate-400">
          RICH_TEXT · 双栏对比，左侧为基准版本，右侧为目标版本
        </p>
      </header>
      <div className="overflow-x-auto">
        <SideBySideTable sections={sections} from={from} to={to} ariaLabel="正文版本对比" />
      </div>
    </article>
  )
}
    ...
    <article className="rounded-xl border border-slate-200 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-slate-400">
            {section.kind}
            {section.valueType ? ` · ${section.valueType}` : ''}
          </p>
        </div>
      ...
      {hasHunks ? (
        <div className="mt-4 overflow-hidden overflow-x-auto rounded-lg border border-slate-200">
          <SideBySideTable
            sections={[section]}
            from={from}
            to={to}
            ariaLabel={`${section.label}对比`}
          />
        </div>
      ) : (
        <div className="mt-4 grid gap-3 lg:grid-cols-2">
          <div className="rounded-lg border border-red-100 bg-red-50/50 p-3">
            <div className="mb-2 text-xs font-semibold text-red-700">
              <span aria-label="删除或旧值">−</span> 修改前
            </div>
            <Value value={section.before} />
          </div>
          <div className="rounded-lg border border-emerald-100 bg-emerald-50/50 p-3">
            <div className="mb-2 text-xs font-semibold text-emerald-700">
              <span aria-label="新增或新值">+</span> 修改后
            </div>
            <Value value={section.after} />
          </div>
        </div>
      )}
    </article>
...
function groupSections(sections) {
  ...
  return groups.filter(
    (group) => group.section || group.body.some((section) => section.status !== 'UNCHANGED'),
  )
}
    ...
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          ['修改', diff.summary?.modified],
          ['未变化', diff.summary?.unchanged],
        ].map(([label, value]) => (
