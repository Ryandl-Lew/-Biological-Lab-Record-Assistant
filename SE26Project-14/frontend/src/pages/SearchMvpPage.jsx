...
function targetPath(target) {
  ...
  if (target.type === 'RECORD_ATTACHMENT')
    return `/records/${target.id}#attachment-${target.attachmentId}`
  return null
}
...
export default function SearchMvpPage() {
  ...
  const [showFilters, setShowFilters] = useState(
    Boolean(
      ...
      searchParams.get('recordStatus'),
    ),
  )
  const [result, setResult] = useState({
    items: [],
    meta: { page: 0, size: 20, total: 0, counts: {} },
  })
      ...
      <div className="mx-auto max-w-4xl">
        <div className="relative">
          ...
          <button
            ...
            className={`absolute right-2.5 top-1/2 flex h-9 -translate-y-1/2 items-center gap-1.5 rounded-lg px-3 text-sm font-medium ${showFilters ? 'bg-brand-50 text-brand-700' : 'text-slate-500 hover:bg-slate-100'}`}
          >
        ...
        {showFilters && (
          <div className="mt-3 grid grid-cols-1 gap-3 rounded-xl border border-slate-200/60 bg-white/85 p-4 shadow-card backdrop-blur-sm sm:grid-cols-3">
            <div>
              <label htmlFor="search-project" className="field-label">
                项目
              </label>
              <select
                id="search-project"
            ...
            <div>
              <label htmlFor="search-creator" className="field-label">
                记录创建者
              </label>
              <select
                id="search-creator"
                ...
                className="input h-10 disabled:bg-slate-50"
              >
            ...
            <div>
              <label htmlFor="search-status" className="field-label">
                记录状态
              </label>
              <select
                id="search-status"
        ...
        <div className="mt-5 overflow-x-auto">
          <Tabs
            items={tabs}
            activeKey={filters.entityType}
            onChange={(value) => updateFilter('entityType', value === 'ALL' ? '' : value)}
          />
        </div>
        {error && (
        ...
        <div className="mt-4 space-y-2.5">
          {loading ? (
            <div className="rounded-xl border border-slate-200/60 bg-white/85 p-8 text-center text-sm text-slate-400 backdrop-blur-sm">
              搜索中…
            </div>
            ...
            result.items.map((hit) => {
              ...
              return (
                <button
                  ...
                  className="flex w-full items-center gap-4 rounded-xl border border-slate-200/60 bg-white/85 px-4 py-3.5 text-left shadow-card backdrop-blur-sm transition-all hover:border-brand-200 hover:shadow-pop"
                >
                  <div
                    className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${config?.tone || ENTITY_CONFIG.ATTACHMENT.tone}`}
                  >
                    <Icon size={17} />
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    {hit.projectName && (
                      <p className="mt-1 text-[11px] text-slate-400">所属项目：{hit.projectName}</p>
                    )}
                  </div>
