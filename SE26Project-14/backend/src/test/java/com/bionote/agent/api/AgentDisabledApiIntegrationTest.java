...
class AgentDisabledApiIntegrationTest {
    ...
    @Test
    void disabledFeatureReturnsStableErrorWithoutProviderAccess() throws Exception {
        MvcResult registered =
                mvc.perform(
                                post("/api/v1/auth/register")
                                        .contentType(MediaType.APPLICATION_JSON)
                                        .content(
                                                "{\"displayName\":\"Disabled Agent\",\"email\":\"disabled-"
                                                        + UUID.randomUUID()
                                                        + "@example.com\",\"password\":\"Password123!\"}"))
                        .andExpect(status().isOk())
                        .andReturn();
        String token = value(registered, "data.accessToken");
        mvc.perform(
                        post("/api/v1/records/{id}/agent-runs", UUID.randomUUID())
        ...
        mvc.perform(
                        post("/api/v1/records/{id}/agent-chat", UUID.randomUUID())
                                .header("Authorization", "Bearer " + token)
                                .contentType(MediaType.APPLICATION_JSON)
                ...
                .andExpect(jsonPath("$.code").value("AGENT_DISABLED"));
        mvc.perform(
                        post("/api/v1/projects/{id}/agent-chat", UUID.randomUUID())
                                .header("Authorization", "Bearer " + token)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"message\":\"hello\"}"))
                .andExpect(status().isServiceUnavailable())
