...
class AuthProfileIntegrationTest {
    ...
    private String register(String email) throws Exception {
        ...
        return mvc.perform(
                ...
                .andExpect(jsonPath("$.data.user.email").value(email.trim().toLowerCase()))
                .andReturn()
                .getResponse()
                .getContentAsString()
                .split("\\\"accessToken\\\":\\\"")[1]
    ...
    void duplicateEmailIsConflict() throws Exception {
        ...
        mvc.perform(
                        post("/api/v1/auth/register")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        "{\"displayName\":\"另一个\",\"email\":\" user@example.com \",\"password\":\"Password123!\"}"))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.code").value("DUPLICATE_RESOURCE"));
    }

    @Test
    void loginUsesEmailAndRejectsWrongPassword() throws Exception {
    ...
    @Test
    void profileUpdateChangesOnlyDisplayName() throws Exception {
        ...
        mvc.perform(
                        put("/api/v1/users/me")
                                ...
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        "{\"displayName\":\"新姓名\",\"email\":\"hacker@example.com\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.displayName").value("新姓名"))
                .andExpect(jsonPath("$.data.email").value("profile@example.com"));
    }

    @Test
    void invalidAvatarsAreRejected() throws Exception {
    ...
    @Test
    void validPngAvatarCanBeRead() throws Exception {
        ...
        mvc.perform(get("/api/v1/users/" + id + "/avatar"))
                .andExpect(status().isOk())
