...
public class AuditEventHandler implements DomainEventHandler<DomainEvent> {
    ...
    private Target target(DomainEvent event) {
        if (event instanceof RecordRevisionRestoredEvent restored)
            return new Target("RESTORE_OPERATION", restored.restoreOperationId());
        if (event instanceof AgentRunRequestedEvent requested)
            return new Target("AGENT_RUN", requested.runId());
        if (event instanceof AgentRunSucceededEvent succeeded)
            return new Target("AGENT_RUN", succeeded.runId());
        if (event instanceof AgentRunFailedEvent failed)
            return new Target("AGENT_RUN", failed.runId());
        if (event instanceof AgentArtifactViewedEvent viewed)
            return new Target("AGENT_ARTIFACT", viewed.artifactId());
        return null;
    }
