...
interface ExperimentRecordJpaRepository extends JpaRepository<ExperimentRecordEntity, UUID> {
    ...
    @Query(
            ...
            """,
            nativeQuery = true)
    Page<IdProjection> searchVisible(
    ...
    @Query(
            "delete from ExperimentRecordEntity r where r.id=:id and r.creatorId=:creatorId and r.provisional=true")
    ...
    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query(
            value = "UPDATE experiment_records SET field_values_json=:json WHERE id=:id",
            nativeQuery = true)
    ...
    @Query(
            ...
            """,
            nativeQuery = true)
    int markReviewChangesRequested(
            @Param("id") String id, @Param("reviewId") String reviewId, @Param("now") Instant now);
    ...
    @Query(
            ...
            """,
            nativeQuery = true)
    int restoreWorkingCopy(
            @Param("id") String id,
            @Param("expectedVersion") long expectedVersion,
            @Param("title") String title,
            @Param("experimentType") String experimentType,
            @Param("experimentDate") LocalDate experimentDate,
