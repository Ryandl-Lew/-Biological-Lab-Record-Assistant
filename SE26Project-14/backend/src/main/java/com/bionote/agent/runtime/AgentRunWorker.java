...
public class AgentRunWorker {
    ...
    @Scheduled(fixedDelayString = "${agent.worker-poll-ms:1000}")
    public void poll() {
    ...
    public boolean tick() {
        lifecycle.failStaleRunning(Instant.now().minusMillis(properties.getWorkerStaleTimeoutMs()));
        AgentRunRecord run = runs.claimNext();
