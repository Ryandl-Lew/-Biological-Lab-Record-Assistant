...
public final class RecordActionPolicy {
    ...
    public Decision canEdit(RecordContext context, UUID actorId) {
        ...
        if (!"ACTIVE".equals(context.projectStatus()))
            return Decision.deny("PROJECT_ARCHIVED", "项目已归档，只能查看");
        if (!context.creatorId().equals(actorId))
            return Decision.deny("ACCESS_DENIED", "只有记录创建者可以编辑");
        if (!EDITABLE.contains(context.recordStatus()))
            return Decision.deny("RECORD_STATE_CONFLICT", "当前记录状态不可编辑");
    ...
    public Decision canRestore(RecordContext context, UUID actorId) {
        ...
        if (!visible.allowed()) return visible;
        if (!"ACTIVE".equals(context.projectStatus()))
            return Decision.deny("RESTORE_NOT_ALLOWED", "项目已归档，不能恢复记录");
        if (!context.creatorId().equals(actorId))
            return Decision.deny("ACCESS_DENIED", "只有记录创建者可以恢复工作副本");
        if (context.provisional()) return Decision.deny("RESTORE_NOT_ALLOWED", "临时记录没有可恢复的历史修订");
        if (!EDITABLE.contains(context.recordStatus()))
            return Decision.deny("RESTORE_NOT_ALLOWED", "当前记录状态不允许恢复");
        return Decision.allow();
    }

    public Decision canViewRevision(RecordContext context, UUID actorId) {
        return requireVisibleMember(context);
    }
    ...
    public Decision canGenerateRecordSummary(RecordContext context, UUID actorId) {
        ...
        if (!visible.allowed()) return visible;
        if (!context.creatorId().equals(actorId))
            return Decision.deny("ACCESS_DENIED", "只有记录创建者可以生成记录总结");
    ...
    private Decision requireVisibleMember(RecordContext context) {
        if (context == null
                || context.deleted()
                || context.actorProjectRole() == null
                || context.actorProjectRole().isBlank()) {
