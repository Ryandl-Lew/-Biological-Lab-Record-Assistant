...
public class JpaAuditEventAppender implements AuditEventAppender {
    ...
    public JpaAuditEventAppender(AuditEventJpaRepository events, AuditJsonCodec json) {
        this.events = events;
        this.json = json;
    }
    ...
    public void appendOnce(
        ...
        events.appendOnce(
                ...
                json.encode(metadata),
                occurredAt);
        AuditEventEntity stored = events.findById(eventId).orElseThrow();
        if (!eventType.equals(stored.eventType)
                || !targetType.equals(stored.targetType)
                || !targetId.equals(stored.targetId))
