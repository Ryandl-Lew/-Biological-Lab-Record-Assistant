...
public class AttachmentService implements AttachmentUseCase {
    ...
    @Override
    public List<AttachmentDtos.View> list(UUID user, UUID recordId) {
        RecordStore.RecordData record = record(recordId);
        requireMember(user, record);
        return attachments.findActiveByRecord(recordId).stream()
                .map(value -> view(value, user, record, true))
                .toList();
    }

    @Override
    @Transactional
    public AttachmentDtos.View upload(UUID user, UUID recordId, MultipartFile file) {
        RecordStore.RecordData record = record(recordId);
        requireWritable(user, record);
        AttachmentStorage.StoredFile stored = storage.store(file);
        UUID id = UUID.randomUUID();
        Instant now = Instant.now();
        try {
            attachments.insert(
                    new AttachmentStore.AttachmentRecord(
                            id,
                            recordId,
                            user,
        ...
        TransactionSynchronizationManager.registerSynchronization(
                new TransactionSynchronization() {
                    ...
                    public void afterCompletion(int status) {
                        if (status != STATUS_COMMITTED) storage.deleteQuietly(stored.storageKey());
                    }
                });
        events.audit(
                user,
    ...
    public void delete(UUID user, UUID id) {
        ...
        RecordStore.RecordData record = record(attachment.recordId());
        requireWritable(user, record);
    ...
    @Override
    public FilePayload load(UUID user, UUID id, boolean preview) {
        AttachmentStore.AttachmentRecord attachment = attachment(id, false);
        RecordStore.RecordData record = record(attachment.recordId());
        requireMember(user, record);
        if (preview && !isPreviewable(attachment.mediaType()))
            throw new ApiException(
                    HttpStatus.UNSUPPORTED_MEDIA_TYPE, "PREVIEW_NOT_SUPPORTED", "该文件类型仅支持下载");
        return new FilePayload(
                storage.read(attachment.storageKey()),
                attachment.originalFilename(),
                attachment.mediaType());
    }

    @Override
    public List<AttachmentDtos.View> revisionAttachments(UUID user, UUID revisionId) {
        UUID project =
                attachments
                        .findRevisionProjectId(revisionId)
                        .orElseThrow(
                                () ->
                                        new ApiException(
        ...
        return attachments.findByRevision(revisionId).stream()
                .map(value -> view(value, user, null, false))
                .toList();
    }
    ...
    private AttachmentDtos.View view(
            AttachmentStore.AttachmentRecord attachment,
            UUID user,
            RecordStore.RecordData record,
            boolean calculateWritable) {
        String uploaderName =
                users.findById(attachment.uploaderId()).map(User::getDisplayName).orElse("");
        boolean writable = calculateWritable && record != null && isWritable(user, record);
        return new AttachmentDtos.View(
                attachment.id(),
                attachment.recordId(),
                attachment.originalFilename(),
                attachment.mediaType(),
                attachment.sizeBytes(),
                isPreviewable(attachment.mediaType()),
    ...
    private AttachmentStore.AttachmentRecord attachment(UUID id, boolean includeDeleted) {
        return attachments
                ...
                .orElseThrow(
                        () ->
                                new ApiException(
                                        HttpStatus.NOT_FOUND, "RESOURCE_NOT_FOUND", "附件不存在"));
    }
    ...
    private boolean isWritable(UUID user, RecordStore.RecordData record) {
        String projectStatus =
                projects.findById(record.projectId())
                        ...
                        .orElse("");
        return record.creatorId().equals(user)
    ...
    private void requireWritable(UUID user, RecordStore.RecordData record) {
        requireMember(user, record);
        if (!isWritable(user, record))
            throw new ApiException(
                    HttpStatus.CONFLICT, "RECORD_STATE_CONFLICT", "当前记录状态或权限不允许管理附件");
    }
}
