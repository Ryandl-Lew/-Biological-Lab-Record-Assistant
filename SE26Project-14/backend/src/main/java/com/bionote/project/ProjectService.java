...
public class ProjectService implements ProjectUseCase {
    ...
    public ProjectService(
            ...
            ProjectInvitationStore invitations,
            UserRepository users,
            CollaborationEvents events,
            RecordMembershipGuard guard,
    ...
    @Override
    @Transactional
    public ProjectDtos.ProjectView create(UUID userId, ProjectDtos.CreateProjectRequest request) {
        UUID id = UUID.randomUUID();
        Instant now = Instant.now();
        projects.insert(
                new ProjectStore.ProjectRecord(
                        id,
                        request.name().trim(),
                        trim(request.description()),
                        trim(request.detailedDescription()),
                        "ACTIVE",
                        userId,
                        now,
                        now,
    ...
    @Override
    public PagedResponse<ProjectDtos.ProjectView> list(
            UUID userId, String keyword, String status, int page, int size) {
        ...
        List<ProjectDtos.ProjectView> items =
                result.items().stream().map(this::mapProject).toList();
    ...
    public ProjectDtos.ProjectView detail(UUID userId, UUID projectId) {
        ...
        String role =
                members.findRole(projectId, userId)
                        .orElseThrow(
                                () ->
                                        new ApiException(
    ...
    @Override
    @Transactional
    public ProjectDtos.ProjectView archive(UUID userId, UUID projectId) {
    ...
    @Override
    @Transactional
    public ProjectDtos.InvitationView invite(
            UUID userId, UUID projectId, ProjectDtos.InviteRequest request) {
        requireActiveOwner(projectId, userId);
        String email = request.email().trim().toLowerCase(Locale.ROOT);
        User found =
                users.findByEmailNormalized(email)
                        .orElseThrow(
                                () ->
                                        new ApiException(
                                                HttpStatus.NOT_FOUND,
        ...
        if (invitee.equals(userId))
            throw new ApiException(HttpStatus.CONFLICT, "DUPLICATE_RESOURCE", "不能邀请自己");
        ...
        try {
            invitations.insert(
                    new ProjectInvitationStore.InvitationRecord(
                            id,
                            projectId,
                            userId,
                            invitee,
    ...
    @Override
    @Transactional
    public ProjectDtos.ProjectView accept(UUID userId, UUID invitationId) {
        ...
        if (invitations.changePendingStatus(invitationId, "ACCEPTED", now) == 0)
            throw invitationStateConflict();
        events.notify(
                project.ownerId(),
                "INVITATION_ACCEPTED",
                "邀请已接受",
                invitation.inviteeEmailSnapshot() + " 已加入项目",
                Map.of("projectId", projectId.toString()),
    ...
    @Override
    @Transactional
    public void reject(UUID userId, UUID invitationId) {
        ProjectInvitationStore.InvitationRecord invitation = invitationRecord(invitationId);
        validateInvitationActor(invitation, userId);
        expireIfNeeded(invitation, invitationId);
        if (!"PENDING".equals(invitation.status())) throw invitationStateConflict();
        if (invitations.changePendingStatus(invitationId, "REJECTED", Instant.now()) == 0)
            throw invitationStateConflict();
        ...
        events.notify(
                ...
                Map.of("projectId", invitation.projectId().toString()),
                "invitation-rejected:" + invitationId);
        events.audit(
                userId,
    ...
    @Override
    @Transactional
    public ProjectDtos.MemberView changeRole(
            UUID ownerId, UUID projectId, UUID target, ProjectDtos.RoleRequest request) {
        requireActiveOwner(projectId, ownerId);
        ProjectStore.ProjectRecord project = project(projectId);
        if (target.equals(project.ownerId()))
            throw new ApiException(HttpStatus.CONFLICT, "OWNER_IMMUTABLE", "项目负责人角色不可修改");
    ...
    @Transactional
    public void remove(
            UUID ownerId, UUID projectId, UUID target, ProjectDtos.RemoveRequest request) {
        requireActiveOwner(projectId, ownerId);
        ProjectStore.ProjectRecord project = project(projectId);
        if (target.equals(project.ownerId()))
            throw new ApiException(HttpStatus.CONFLICT, "OWNER_IMMUTABLE", "不能移除项目负责人");
        String current = requireMember(projectId, target);
        var blockers = guard.recordsBlockingMemberChange(projectId, target);
        if (!blockers.isEmpty())
            throw new ApiException(
    ...
    private ProjectDtos.ProjectView mapProject(ProjectStore.ProjectAccess access) {
        ...
        return new ProjectDtos.ProjectView(
                project.id(),
                project.name(),
                project.description(),
                ...
                Map.of(
                        ...
                        active && owner,
                        "canCreateRecord",
                        active && !"REVIEWER".equals(role)),
                members.count(project.id()),
                projects.recordCount(project.id()));
    }

    private ProjectDtos.MemberView memberView(ProjectMemberStore.MemberRecord member) {
        return new ProjectDtos.MemberView(
                member.userId(),
                member.displayName(),
                member.email(),
                member.avatarStorageKey() == null
                        ? null
                        : "/api/v1/users/" + member.userId() + "/avatar",
                member.role(),
                member.joinedAt(),
                member.lastActiveAt(),
                Map.of(
    ...
    private ProjectStore.ProjectRecord project(UUID id) {
        return projects.findById(id)
                .orElseThrow(
                        () ->
                                new ApiException(
                                        HttpStatus.NOT_FOUND, "RESOURCE_NOT_FOUND", "项目不存在"));
    }
    ...
    private ProjectInvitationStore.InvitationRecord invitationRecord(UUID id) {
        return invitations
                ...
                .orElseThrow(
                        () ->
                                new ApiException(
                                        HttpStatus.NOT_FOUND, "RESOURCE_NOT_FOUND", "邀请不存在"));
    }
    ...
    private ProjectDtos.InvitationView invitation(UUID id) {
        ...
        return new ProjectDtos.InvitationView(
                id,
                invitation.projectId(),
                project(invitation.projectId()).name(),
                invitation.inviteeUserId(),
                invitation.inviteeEmailSnapshot(),
                invitation.status(),
    ...
    private String requireMember(UUID projectId, UUID userId) {
        return members.findRole(projectId, userId)
                .orElseThrow(
                        () ->
                                new ApiException(
                                        HttpStatus.NOT_FOUND, "RESOURCE_NOT_FOUND", "项目不存在或无权访问"));
    }

    private void requireOwner(String role) {
        if (!"OWNER".equals(role))
            throw new ApiException(HttpStatus.FORBIDDEN, "ACCESS_DENIED", "仅项目负责人可执行此操作");
    }
    ...
    private void validateInvitationActor(
            ProjectInvitationStore.InvitationRecord invitation, UUID userId) {
    ...
    private void expireIfNeeded(ProjectInvitationStore.InvitationRecord invitation, UUID id) {
        if ("PENDING".equals(invitation.status())
                && !invitation.expiresAt().isAfter(Instant.now())) {
