...
interface ProjectMemberJpaRepository extends JpaRepository<ProjectMemberEntity, ProjectMemberId> {
    ...
    @Query(
            "update ProjectMemberEntity pm set pm.lastActiveAt=:now where pm.id.projectId=:projectId and pm.id.userId=:userId")
    int touch(
    ...
    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query(
            "update ProjectMemberEntity pm set pm.role=:role where pm.id.projectId=:projectId and pm.id.userId=:userId")
    int updateRole(
            @Param("projectId") UUID projectId,
            @Param("userId") UUID userId,
