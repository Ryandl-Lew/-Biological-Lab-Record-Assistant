...
所有真实 LLM 请求统一读取仓库根目录 `llm` 中的 `base_url (OpenAI)`、`api_key` 和 `model`，不再提供用户级模型选择或自备 Key。默认 `local` profile 已启用真实 `openai-compatible` provider；服务器部署可将当前数据库 profile 与 `real-llm` profile 组合启用。后端从 `backend/` 启动时会自动读取 `../llm`；服务器推荐通过 `LLM_CONFIG_PATH` 指向只读挂载的同格式文件。地址和密钥在模型调用前重新读取，新建任务使用当前模型名；仅轮换密钥通常无需重启，跨服务商迁移应先清空旧任务。真实 provider smoke 默认禁用，设置 `RUN_REAL_AGENT_SMOKE=true` 后运行 `mvnw -Dtest=RealProviderSmokeTest test`；该测试同样读取 `llm`，且只发送合成数据。OpenAI-compatible adapter 会把严格输出 Schema 加入 system instruction。详细迁移方式见 `../docs/LLM_DEPLOYMENT.md`。
