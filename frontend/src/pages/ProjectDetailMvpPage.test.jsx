...
vi.mock('@/api', () => ({
  ...
  fetchProjectAuditEvents: vi.fn().mockResolvedValue({
    items: [
      {
        id: 'e1',
...
describe('ProjectDetailMvpPage', () => {
  it('fetches all audit events and renders the timeline graph', async () => {
    render(
      <MemoryRouter initialEntries={['/projects/p1?tab=timeline']}>
        <Routes>
    ...
    await waitFor(() =>
      expect(fetchProjectAuditEvents).toHaveBeenCalledWith('p1', { page: 0, size: 100 }),
    )
  })
