        ...
        <aside className="border-b border-slate-200 bg-slate-50/60 p-4 lg:border-b-0 lg:border-r">
          <div className="relative">
            ...
            <input
              ...
              className="input h-10 bg-white pl-9"
              placeholder="搜索标题、编号、目的"
            />
          </div>
          <select
            aria-label="记录状态"
            ...
            className="input mt-3 h-10 bg-white"
          >
          ...
          <div className="mt-4 flex items-center justify-between px-2 text-xs font-medium uppercase tracking-wider text-slate-400">
            <span>项目与记录</span>
          ...
          <div className="mt-2 max-h-[540px] overflow-y-auto pr-1">
            {loading ? (
              ...
              <EmptyState icon={FileText} title="暂无匹配记录" />
            )}
          </div>
        </aside>
        <main className="min-w-0 p-6 lg:p-8">
          {selected ? (
            <div className="mx-auto max-w-4xl">
              <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-6">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <StatusBadge kind="record" status={selected.status} />
                    <Badge>{selected.experimentType}</Badge>
                  </div>
                  <h2 className="mt-3 text-2xl font-bold text-slate-900">{selected.title}</h2>
                  <p className="mt-2 text-sm text-slate-500">
                    {selected.projectName} · {selected.code}
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  {selected.capabilities?.canSubmit && (
                  ...
                  {selected.capabilities?.canEdit && (
                    <Button
                      variant="secondary"
                      icon={Pencil}
                      onClick={() => navigate(`/records/${selected.id}/edit`)}
                    >
                      编辑记录
                    </Button>
                  )}
              ...
              <dl className="grid gap-3 border-b border-slate-100 py-6 sm:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-lg bg-slate-50 p-3">
                  <dt className="flex items-center gap-1.5 text-xs text-slate-400">
                    <UserRound size={13} />
                    创建者
                  </dt>
                  <dd className="mt-1 text-sm font-medium">{selected.creatorName}</dd>
                </div>
                <div className="rounded-lg bg-slate-50 p-3">
                  <dt className="flex items-center gap-1.5 text-xs text-slate-400">
                  ...
                  <dd className="mt-1 text-sm font-medium">{selected.experimentDate}</dd>
                </div>
                <div className="rounded-lg bg-slate-50 p-3">
                  <dt className="flex items-center gap-1.5 text-xs text-slate-400">
                    <Clock3 size={13} />
              ...
              <section className="border-t border-slate-100 pt-6">
                ...
                {selected.contentHtml ? (
                  <div
                    ...
                    dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(selected.contentHtml) }}
                  />
                ) : (
                  <p className="mt-3 text-sm text-slate-400">暂无正文内容</p>
                )}
              </section>
            </div>
          ) : (
            !loading && (
              <div className="flex h-full items-center justify-center">
                <EmptyState icon={FileText} title="选择一条记录查看简介" />
              </div>
            )
          )}
        </main>
