...
function rowY(row, totalRows) {
  return (totalRows - 1 - row) * ROW_HEIGHT + ROW_HEIGHT / 2
}
...
export default function TimelineGraph({
  ...
  const dimmedIds = new Set(
    activeFilters
      ? nodes.filter((n) => !n.events.some((e) => filteredIds.has(e.id))).map((n) => n.id)
      : [],
  )

  const displayNodes = [...nodes].reverse()
  const matchingCount = activeFilters
    ? displayNodes.filter((n) => !dimmedIds.has(n.id)).length
    : null
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-5">
        <div>
          <label htmlFor="tl-type" className="field-label">
            事件类型
          </label>
          <select
            id="tl-type"
        ...
        <div>
          <label htmlFor="tl-actor" className="field-label">
            操作者
          </label>
          <select
            id="tl-actor"
        ...
        <div>
          <label htmlFor="tl-from" className="field-label">
            开始日期
          </label>
          <div className="relative">
            <input
              ...
              className="input h-10 pr-3"
            />
            {!filters.from && (
              <span
                className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 bg-white pr-4 text-sm text-slate-400"
                aria-hidden="true"
              >
                年 / 月 / 日
              </span>
        ...
        <div>
          <label htmlFor="tl-to" className="field-label">
            结束日期
          </label>
          <div className="relative">
            <input
      ...
      {activeFilters && matchingCount != null && (
        <p className="rounded-lg bg-brand-50 px-4 py-2 text-sm text-brand-700">
          筛选结果：<span className="font-semibold">{matchingCount}</span>{' '}
          个匹配节点（淡化项为不匹配）
        </p>
      )}

      {!nodes.length ? (
        <div className="flex h-32 items-center justify-center rounded-xl border text-sm text-slate-400">
          {emptyMessage}
        </div>
      ) : (
        <div
          ...
          <div className="relative">
              ...
              {merges.map((merge, i) => {
                ...
                return (
                  <path
                    ...
                    d={d}
                    fill="none"
                    stroke={merge.from.color.stroke}
                    strokeWidth={1.5}
                    opacity={0.55}
                  />
                )
            ...
            <ol className="m-0 list-none p-0" style={{ marginLeft: gutterWidth }}>
              {displayNodes.map((node) => {
                ...
                const time = new Date(firstEvent.createdAt).toLocaleString('zh-CN', {
                  month: 'numeric',
                  day: 'numeric',
                  hour: '2-digit',
                  minute: '2-digit',
                ...
                return (
                  ...
                  >
                    <div
                      className={`flex w-full max-w-4xl gap-3 rounded-xl border border-slate-200/60 bg-white/85 py-2.5 px-4 backdrop-blur-sm transition hover:border-brand-200 hover:bg-slate-50`}
                    >
                      {Icon ? (
                        <span
                          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs ${node.color.light}`}
                          style={{ color: node.color.stroke }}
                        >
                          <Icon size={16} />
                        </span>
                      ) : (
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm leading-6">
                          <span className="font-semibold text-slate-900">
                            {firstEvent.actorName}
                          </span>
                          <span className="text-slate-600"> {label}</span>
                          {count}
                        </p>
                        {context && (
                          <p className="mt-0.5 truncate text-xs text-slate-400">{context}</p>
                        )}
                      </div>
