...
describe('RevisionDiffViewer', () => {
  it('renders structured scalar and multi-line text changes', () => {
    render(<RevisionDiffViewer diff={{ generatedAt: '2026-07-27T00:00:00Z', truncated: true, warnings: ['正文已截断'], summary: { added: 1, removed: 1, modified: 2, unchanged: 0 }, sections: [
      { key: 'fixed:title', label: '标题', kind: 'SCALAR', status: 'MODIFIED', before: '旧标题', after: '新标题', textHunks: [] },
      { key: 'field:notes', label: '备注', kind: 'TEMPLATE_FIELD', valueType: 'MULTI_LINE_TEXT', status: 'MODIFIED', before: '旧备注', after: '新备注', textHunks: [{ operation: 'DELETE', text: '旧' }, { operation: 'INSERT', text: '新' }, { operation: 'EQUAL', text: '备注' }] },
      { key: 'review', label: '审核信息', kind: 'REVIEW_METADATA', status: 'MODIFIED', before: '退回', after: '通过', textHunks: [] },
    ] }} />)
  ...
  it('renders rich text body as a GitHub style side-by-side diff with context and word highlights', () => {
    render(<RevisionDiffViewer diff={{
      generatedAt: '2026-07-27T00:00:00Z', truncated: false, warnings: [],
      from: { type: 'REVISION', revisionId: 'r1', revisionNo: 1 }, to: { type: 'WORKING_COPY' },
      summary: { added: 1, removed: 1, modified: 1, unchanged: 1 },
      sections: [
        { key: 'content:block:0', label: '正文块 1', kind: 'RICH_TEXT', status: 'UNCHANGED', before: '相同的段落', after: '相同的段落', textHunks: [] },
        { key: 'content:block:1', label: '正文块 2', kind: 'RICH_TEXT', status: 'MODIFIED', before: '旧的结论', after: '新的结论', textHunks: [{ operation: 'DELETE', text: '旧' }, { operation: 'INSERT', text: '新' }, { operation: 'EQUAL', text: '的结论' }] },
        { key: 'content:block:2', label: '正文块 3', kind: 'RICH_TEXT', status: 'ADDED', before: null, after: '补充的段落', textHunks: [] },
        { key: 'content:block:3', label: '正文块 4', kind: 'RICH_TEXT', status: 'REMOVED', before: '删除的段落', after: null, textHunks: [] },
      ],
    }} />)
  ...
  it('keeps the body comparison in section order and hides attachment and review sections', () => {
    render(<RevisionDiffViewer diff={{
      generatedAt: '2026-07-27T00:00:00Z', truncated: false, warnings: [],
      from: { type: 'REVISION', revisionId: 'r1', revisionNo: 1 }, to: { type: 'REVISION', revisionId: 'r2', revisionNo: 2 },
      summary: { added: 0, removed: 0, modified: 3, unchanged: 0, attachmentAdded: 1, attachmentRemoved: 1 },
      sections: [
        { key: 'fixed:title', label: '实验名称', kind: 'SCALAR', status: 'MODIFIED', before: '旧题', after: '新题', textHunks: [] },
        { key: 'content:block:0', label: '正文块 1', kind: 'RICH_TEXT', status: 'MODIFIED', before: '旧正文', after: '新正文', textHunks: [{ operation: 'DELETE', text: '旧正文' }, { operation: 'INSERT', text: '新正文' }] },
    ...
    expect(screen.getByText('实验名称').compareDocumentPosition(body)).toBe(Node.DOCUMENT_POSITION_FOLLOWING)
