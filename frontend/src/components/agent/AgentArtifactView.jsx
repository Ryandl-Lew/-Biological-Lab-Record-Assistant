...
export default function AgentArtifactView({ artifact, run, onEvidence }) {
  ...
  return <div className="space-y-5">
    <Surface title={content.headline || 'Agent 报告'} extra={<div className="flex flex-wrap gap-2"><Badge>{artifact.artifactKind}</Badge>{run && <Badge>{run.provider} / {run.model}</Badge>}</div>}>
      <p className="whitespace-pre-wrap text-sm leading-7 text-slate-700">{content.executiveSummary}</p>
      <div className="mt-4 flex flex-wrap gap-x-5 gap-y-1 text-xs text-slate-400"><span>生成于 {new Date(artifact.createdAt).toLocaleString()}</span><span>报告范围 {content.period?.start} 至 {content.period?.end}</span>{run?.promptVersionId && <span>Prompt {run.promptVersionId}</span>}</div>
    </Surface>
    <Section title="进展与版本演进" items={content.progress} evidence={content.evidence} onEvidence={onEvidence} />
    <Section title="风险与阻塞" items={content.risks} evidence={content.evidence} onEvidence={onEvidence} renderMeta={(item) => <Badge tone={item.severity === 'HIGH' ? 'red' : item.severity === 'MEDIUM' ? 'amber' : 'gray'}>{item.severity}</Badge>} />
