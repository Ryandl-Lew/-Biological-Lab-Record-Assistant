export function agentErrorMessage(error) {
  ...
  if (
    ['AGENT_UNKNOWN_TOOL', 'AGENT_TOOL_NOT_ALLOWED', 'AGENT_TOOL_ARGUMENTS_INVALID'].includes(code)
  )
    return '模型请求了未注册或非法的工具，运行已安全终止。'
  if (['AGENT_INVALID_OUTPUT', 'AGENT_EVIDENCE_INVALID'].includes(code))
    return '模型结果未通过结构或证据校验，因此没有保存不可靠报告。'
  if (code === 'AGENT_RATE_LIMITED') return '请求过于频繁或并发运行已达上限，请稍后重试。'
  if (code === 'AGENT_LIMIT_EXCEEDED')
    return '模型调用或工具步数已达上限，未生成报告；可稍后重新运行。'
