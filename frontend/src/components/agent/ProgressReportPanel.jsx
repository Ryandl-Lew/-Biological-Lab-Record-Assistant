...
export default function ProgressReportPanel({ project, runId, onRunId, onEvidence }) {
  const [periodStart, setPeriodStart] = useState(date(-7)),
    [periodEnd, setPeriodEnd] = useState(date())
  const [artifacts, setArtifacts] = useState([]),
    [artifact, setArtifact] = useState(null),
    [artifactRun, setArtifactRun] = useState(null),
    [error, setError] = useState(''),
    [creating, setCreating] = useState(false),
    [traceRun, setTraceRun] = useState(null)
  const load = useCallback(async () => {
    try {
      const result = await fetchProjectArtifacts(project.id)
      setArtifacts(result.items)
      if (result.items[0]) {
        const detail = await fetchAgentArtifact(result.items[0].id)
        setArtifact(detail)
        setArtifactRun(await fetchAgentRun(detail.runId))
      }
      setError('')
    } catch (requestError) {
  ...
  const create = async () => {
    ...
    try {
      const run = await createProjectAgentRun(
        project.id,
        {
          artifactKind: AGENT_ARTIFACT_KIND.PROJECT_PROGRESS,
    ...
    <div className="space-y-5">
      ...
      >
        <p className="text-sm text-slate-500">
          报告截止时间与实时项目状态分开显示；`COMPLETED` 不代表实验结果成功。
        </p>
          ...
          <div className="divide-y">
            {artifacts.map((item) => (
              ...
              >
                ...
                <span className="text-brand-600">查看</span>
              </button>
            ))}
          </div>
        </Surface>
      ...
      <Surface title="项目问答">
        <p className="text-sm text-slate-500">
          项目级对话、拟合与过程分析已移至侧栏「Agent助手」。当前项目可一键跳转并自动选中。
        </p>
        <div className="mt-3">
          <a
            href={`/agent?projectId=${project.id}`}
