...
export default function ProgressReportPanel({ project, runId, onRunId, onEvidence }) {
  ...
  return <div className="space-y-5">
    ...
    <Surface title="项目问答">
      <p className="text-sm text-slate-500">项目级对话、拟合与过程分析已移至侧栏「Agent助手」。当前项目可一键跳转并自动选中。</p>
      <div className="mt-3">
        <a href={`/agent?projectId=${project.id}`} className="text-sm font-medium text-brand-600 hover:text-brand-700">
          打开 Agent助手 →
        </a>
      </div>
    </Surface>
