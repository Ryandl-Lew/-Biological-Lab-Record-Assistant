...
test('object-level authorization blocks outsiders and other creators', async ({ request }) => {
  ...
  const outsiderProject = await request.get(`http://127.0.0.1:8080/api/v1/projects/${project.id}`, {
    headers: { Authorization: `Bearer ${outsider.token}` },
  })
  expect([403, 404]).toContain(outsiderProject.status())
  const outsiderRecord = await request.get(`http://127.0.0.1:8080/api/v1/records/${record.id}`, {
    headers: { Authorization: `Bearer ${outsider.token}` },
  })
